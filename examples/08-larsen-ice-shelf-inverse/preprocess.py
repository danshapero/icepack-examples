...
def _interpolate(q, i, j, m):
    weight = 0.0
    Q = 0.0
    ny, nx = np.shape(q.data)
    for k in range(max(i - m + 1, 0), min(i + m, ny)):
        for l in range(max(j - m + 1, 0), min(j + m, nx)):
...
if __name__ == "__main__":
    ...
    dx, dy = q.x[1] - q.x[0], q.y[1] - q.y[0]
    jmin = math.floor((np.min(mesh.x) - q.x[0]) / dx)
    jmax = math.ceil((np.max(mesh.x) - q.x[0]) / dx)
    imin = math.floor((np.min(mesh.y) - q.y[0]) / dy)
    imax = math.ceil((np.max(mesh.y) - q.y[0]) / dy)

    q = GridData(q.x[jmin:jmax], q.y[imin:imax],
                 q.data[imin:imax, jmin:jmax], q.missing)
